...
For individual services, enter the declared environment inside the pane command, such as `direnv exec "$project_root" <service-command>` or `devenv shell -- <service-command>`. A noninteractive pane does not reliably execute interactive shell hooks. Prefer the entry point that later project commands will use, because entry points can differ in inherited variables: devenv 2.3's `devenv shell` clears `TMPDIR`, which `direnv exec` keeps. Tools that coordinate processes through the temporary directory then miss each other: builds can overlap, and a code reloader that relies on compile notifications keeps serving stale modules. When the entry points must differ, pass the shared variable in the pane command, such as `devenv shell -- env TMPDIR="$TMPDIR" <service-command>`. Additional service windows belong to the same worktree session. Keep short commands outside tmux unless their task requires a persistent terminal.
