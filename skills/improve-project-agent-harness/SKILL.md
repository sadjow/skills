---
name: improve-project-agent-harness
description: "Audit or improve a repository's coding-agent instructions, skills, tools, and checks. Use for deliberate project-harness maintenance rather than ordinary feature work or test-harness changes."
---

# Improve Project Agent Harness
...
For an audit of skill instructions, names, or creators, research current official
skill recommendations on every audit. Use `$upgrade-agent-skills` when available
for a collection-focused audit; otherwise fetch the current specification and
relevant provider guidance directly and record the sources and review date.
...
6. Use specialized agents when independent judgment, context isolation, or parallel investigation materially improves the result.
...
Give agents a bounded question and the minimum raw evidence needed. Keep deterministic enforcement outside the model whenever practical.
...
The current agent owns integration and final verification. Delegated work stays
within the task's scope and external-action permissions.
