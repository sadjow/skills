...
- The leading-edge trigger for a clamped drag comes from a reproduced failure and geometry, not a cited source. A center-to-center comparison left a clamped row that was taller than the end row unable to reach the last position. NN/g's center-to-edge trigger fails the same way once the row is more than twice the end row's height, and it agrees with the leading-edge trigger for rows of equal height.
...
in both. Another review found that a drag clamped to the list could not carry a
row past a shorter end row. The skills collection had no owner for reordering;
its only examples used deprecated ARIA drag attributes. The portable method
belongs in this on-demand skill.
...
| Rows of mixed height, with the drag clamped to the list | The leading edge is compared with neighbor centers, so the tallest row reaches both ends | Transfers to wrapped or variable-height rows |
| Unclamped drag of equal-height rows | Either trigger form; both swap after half a row | Stays silent where clamping is absent |
