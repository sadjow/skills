---
name: external-review-prompt
description: Create self-contained, paste-ready prompts for external LLM code review when the reviewer will not have repository, GitHub, terminal, or file-system access. Use when the user asks to package a branch, pull request, diff, codebase context, surrounding non-diff code, test output, or review instructions into one prompt for a larger LLM or another reviewer.
---
